...
# cut -f 29,35,59,84 ../../software/dbNSFP2.9.txt > dbNSFP_narrow
grep -v '1000Gp1' dbNSFP_narrow | awk '{ if ($1 != "." && $2 != "." && $3 != "." && $4 > 0 ) print }' | grep -P 'D.*?\t[PD].*?\tD.*?\t' - > true_class
grep -v '1000Gp1' dbNSFP_narrow | awk '{ if ($1 != "." && $2 != "." && $3 != "." && $4 > 0 ) print }' | grep -v -P 'D.*?\t[PD].*?\tD.*?\t' - > false_class
...
# Making final table
cut -f 2 FC.cleaned | paste TC.cleaned - > final.dbNSFP.classified.cleaned
